...
int reopen_socket(const char *host, uint16_t port)
{
    ...
    //----------------------------------------------------------------------------
    for (int try = 0; try < MAX_REOPEN_TRIES; try++)
    {
...
int submit_request(int sock, const char *host, uint16_t port, char *resource)
{
    ...
    // from the server
    //
    // YOUR ANSWER:  Removing the length of the header tells us the exact of amount of data we received at a particular time, removing that amount of particular data from content_len allows us to know how much data we still need to receive to have the full content.
    //
    //--------------------------------------------------------------------------------
    ...
    while (bytes_remaining > 0)
    {
        ...
        //-----------------------------------------------------------------------------
        bytes_recvd = recv(sock, recv_buff + total_bytes, bytes_remaining, 0); // replace with a valid recv(...); call
    ...
    //
    // YOUR ANSWER:  This function will return the active socket at this point because it did not encounter any errors in reopening the socket, sending our data, receiving initial data, verifing the HTTP header,  and finally if there are no errors receiving the rest of the data.
    //
    //--------------------------------------------------------------------------------
    return sock;
}
